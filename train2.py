...
import time
import os
from model_squre import two_view_net, three_view_net
...
parser.add_argument('--batchsize', default=8, type=int, help='batchsize')   # 8
parser.add_argument('--stride', default=2, type=int, help='stride')  # 步长
...
parser.add_argument('--alpha', default=0.9, type=float, help='weight ratio')     ###
parser.add_argument('--moving_avg', default=1.0, type=float, help='moving average')
parser.add_argument('--droprate', default=0.5, type=float, help='drop rate')  # 下降率
parser.add_argument('--DA', action='store_true', help='use Color Data Augmentation')
parser.add_argument('--resume', action='store_true', help='use resume trainning')
parser.add_argument('--share', action='store_true', help='share weight between different view')  # 共享权重
parser.add_argument('--extra_Google', action='store_true', help='using extra noise Google')
parser.add_argument('--fp16', action='store_true',
                    help='use float16 instead of float32, which will save about 50% memory')
parser.add_argument('--PCB', action='store_true', help='use PCB')
# extra loss 默认为交叉熵损失
parser.add_argument('--instance', action='store_true', help='use instance loss')  ## 修改
...
parser.add_argument('--dc', action='store_true', help='use dc loss')
...
transform_train_list = [
    ...
    transforms.Resize((opt.h, opt.w), interpolation=3),  # 重新定义图片大小
...
]  # 图像归一化,第一个参数矩阵是每个通道的均值序列，第二个参数矩阵是每个通道的标准偏差序列
...
if opt.erasing_p > 0:
    ...
    transform_satellite_list = transform_satellite_list + [RandomErasing(probability=opt.erasing_p, mean=[0.0, 0.0, 0.0])]
...
data_transforms = {
    ...
    'satellite': transforms.Compose(transform_satellite_list)}  # compose()类会将transforms列表里面的transform操作进行遍历
...
# print(image_datasets['satellite'])
# image_datasets['street'] = datasets.ImageFolder(os.path.join(data_dir, 'street'),
#                                                 # os.path.join()函数将两个路径拼接在一起，返回的是拼接好的路径
#                                                 data_transforms['train'])
image_datasets['drone_gai'] = datasets.ImageFolder(os.path.join(data_dir, 'drone_gai'),
                                               data_transforms['train'])
...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
               ...
               for x in ['satellite', 'drone_gai']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['satellite',  'drone_gai']}
class_names = image_datasets['satellite'].classes  # 修改
print(class_names)
print(dataset_sizes)
...
def train_model(model, model_test, criterion, optimizer, scheduler, num_epochs=25):
    ...
    warm_iteration = round(dataset_sizes['satellite'] / opt.batchsize) * opt.warm_epoch  # first 5 epoch dataset_sizes['satellite']
    ...
    if opt.dc:
        # miner = miners.MultiSimilarityMiner()
        criterion_dc = selfloss.dc(lambda_val=0.2, num_classes=701)
    if opt.lifted:
    ...
    if opt.instance:
        criterion_instance = losses.InstanceLoss(gamma=32)
    ...
    for epoch in range(num_epochs - start_epoch):
        ...
        for phase in ['train']:
            ...
            for data, data2 in zip(dataloaders['satellite'], dataloaders['drone_gai']):
                # get the inputs
                inputs, labels = data  # 取出batchsize数据和标签
                ...
                if phase == 'val':
                    ...
                    if opt.views == 2:
                        if opt.PCB:
                            part_pre1, glo_y1, glo_l4idpre1, glo_l3idpre1, x_l3_a1, x_l4_a1 = model(inputs)
                            part_pre2, glo_y2, glo_l4idpre2, glo_l3idpre2, x_l3_a2, x_l4_a2 = model(inputs2)
                        else:
                            outputs, outputs2, outputs_r, outputs_c, outputs2_r, outputs2_c = model(inputs, inputs2)
                            # outputs, outputs2 = model(inputs, inputs2)
                        # print(outputs)
                    # elif opt.views == 3:
                    #     if opt.extra_Google:
                ...
                return_feature = opt.arcface or opt.cosface or opt.circle or opt.triplet or opt.contrast or opt.lifted or opt.sphere or opt.instance or opt.dc
                sm = nn.Softmax(dim=1)
                ...
                if opt.views == 2:
                    if return_feature:
                        ...
                        # ff = ff.div(fnorm.expand_as(ff))  # 8*512,tensor  离散数据转换为向量
                        # ff2 = ff2.div(fnorm2.expand_as(ff2))
                        # loss = criterion(logits, labels) + criterion(logits2, labels2)  # cross
                        # _, preds = torch.max(logits.data, 1)
                        # _, preds2 = torch.max(logits2.data, 1)

                        logits, ff = outputs
                        logits_r, ff_r = outputs_r
                        logits_c, ff_c = outputs_c
                        logits2, ff2 = outputs2
                        logits2_r, ff2_r = outputs2_r
                        logits2_c, ff2_c = outputs2_c
                        fnorm = torch.norm(ff, p=2, dim=1, keepdim=True)
                        fnorm_r = torch.norm(ff_r, p=2, dim=1, keepdim=True)
                        fnorm_c = torch.norm(ff_c, p=2, dim=1, keepdim=True)
                        fnorm2 = torch.norm(ff2, p=2, dim=1, keepdim=True)
                        fnorm2_r = torch.norm(ff2_r, p=2, dim=1, keepdim=True)
                        fnorm2_c = torch.norm(ff2_c, p=2, dim=1, keepdim=True)
                        ff = ff.div(fnorm.expand_as(ff))  # 8*512,tensor  离散数据转换为向量
                        ff_r = ff_r.div(fnorm_r.expand_as(ff_r))  # 8*512,tensor  离散数据转换为向量
                        ff_c = ff_c.div(fnorm_c.expand_as(ff_c))  # 8*512,tensor  离散数据转换为向量
                        ff2 = ff2.div(fnorm2.expand_as(ff2))
                        ff2_r = ff2_r.div(fnorm2_r.expand_as(ff2_r))
                        ff2_c = ff2_c.div(fnorm2_c.expand_as(ff2_c))
                        _, preds = torch.max(logits.data, 1)
                        _, preds2 = torch.max(logits2.data, 1)
                        loss = criterion(logits_r, labels) + criterion(logits_c, labels) + criterion(logits2_r, labels2) + criterion(logits2_c, labels2)  # cross
                        ...
                        if opt.arcface:
                            if opt.loss_merge:
                                ...
                                # loss += criterion_arcface(ff_r, labels)/now_batch_size + criterion_arcface(ff_c, labels)/now_batch_size + criterion_arcface(ff2_r, labels2)/now_batch_size + criterion_arcface(ff2_c, labels2)/now_batch_size
                                loss += criterion_arcface(ff, labels)/now_batch_size + criterion_arcface(ff2, labels2)/now_batch_size
                            # loss_a = criterion_arcface(ff, labels) + criterion_arcface(ff2, labels2)
                                # if opt.extra_Google:
                        ...
                        if opt.cosface:
                            if opt.loss_merge:
                                ...
                                loss += criterion_cosface(ff, labels) + criterion_cosface(ff2, labels2)  # /now_batch_size
                                # if opt.extra_Google:
                        ...
                        if opt.circle:
                            if opt.loss_merge:
                                ...
                                loss += criterion_circle(*convert_label_to_similarity(ff, labels)) / now_batch_size + criterion_circle(
                                    *convert_label_to_similarity(ff2, labels2)) / now_batch_size
                                # if opt.extra_Google:
                                #     loss += criterion_circle(
                                #         *convert_label_to_similarity(ff4, labels4)) / now_batch_size
                        if opt.triplet:
                            if opt.loss_merge:
                                ...
                                # loss += criterion_triplet(ff, labels, hard_pairs) + criterion_triplet(ff2, labels2, hard_pairs2)  # /now_batch_size
                                loss += criterion_triplet(ff_r, labels, hard_pairs) + criterion_triplet(ff2_r, labels2, hard_pairs2) + criterion_triplet(ff_c, labels, hard_pairs) + criterion_triplet(ff2_c, labels2, hard_pairs2)
                                # if opt.extra_Google:
                                #     hard_pairs4 = miner(ff4, labels4)
                                #     loss += criterion_triplet(ff4, labels4, hard_pairs4)
                        if opt.dc:
                            loss += criterion_dc(logits) + criterion_dc(logits2)
                        if opt.lifted:
                            if opt.loss_merge:
                                loss += criterion_lifted(ff_all, labels_all)
                            else:
                                loss += criterion_lifted(ff, labels) + criterion_lifted(ff2, labels2)  # /now_batch_size
                                # if opt.extra_Google:
                        ...
                        #     else:
                        #         loss += criterion_contrast(ff, labels) + criterion_contrast(ff2, labels2)  # /now_batch_size
                        #         # if opt.extra_Google:
                        #         #     loss += criterion_contrast(ff4, labels4)
                        if opt.contrast:
                            # loss += criterion_contrast(ff, labels) + criterion_contrast(ff2, labels2)
                            loss += criterion_contrast(ff_r, labels)/now_batch_size + criterion_contrast(ff_c, labels)/now_batch_size + criterion_contrast(ff2_r, labels2)/now_batch_size + criterion_contrast(ff2_c, labels2)/now_batch_size
                        ...
                        if opt.instance:
                            # loss += criterion_instance(ff, labels) / now_batch_size + criterion_instance(ff2, labels2) / now_batch_size
                            # loss += criterion_instance(ff_r, labels) / now_batch_size + criterion_instance(ff_c, labels) / now_batch_size + criterion_instance(ff2_r, labels2) / now_batch_size + criterion_instance(ff2_c, labels2) / now_batch_size
                            loss += 0
                        #     in_loss = criterion_instance(ff, labels) / now_batch_size + criterion_instance(ff2, labels2) / now_batch_size
                        # loss += loss_in * opt.alpha + con_loss * (1 - opt.alpha)
                ...
                # if opt.views == 3:
                #     running_corrects3 += float(torch.sum(preds2 == labels2.data))
            ...
            # deep copy the model
            if phase == 'train':
                scheduler.step()  # scheduler用来调整学习率
...
if opt.views == 2:
    model = two_view_net(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool,
                         share_weight=opt.share, circle=return_feature, ratio=0.5)   #
elif opt.views == 3:
...
# model = PCB(len(class_names), test=False)   ###修改
...
if start_epoch >= 40:
    opt.lr = opt.lr * 0.1
if start_epoch >= 100:
    opt.alpha = 0.4 * opt.alpha

if not opt.PCB:
    ignored_params = list(map(id, model.classifier_2.parameters()))
    base_params = filter(lambda p: id(p) not in ignored_params, model.parameters())
    optimizer_ft = optim.SGD([
             {'params': base_params, 'lr': 0.1*opt.lr},
             {'params': model.classifier_2.parameters(), 'lr': opt.lr}
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./train2.py', dir_name + '/train2.py')
    copyfile('./model.py', dir_name + '/model_squre.py')
    # save opts
...
triplet_loss = selfloss.TripletLoss()
