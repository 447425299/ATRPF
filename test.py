...
parser.add_argument('--h', default=384, type=int, help='height')  ##修改
parser.add_argument('--w', default=384, type=int, help='width')   ##修改
...
if opt.multi:
    image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms) for x in ['query_drone_new', 'gallery_satellite']} # 'query_satellite', 'gallery_drone',
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=False, num_workers=16) for x in ['query_drone_new', 'gallery_satellite']}    #'query_satellite', 'gallery_drone', 'gallery', 'query', 'multi-query'
else:
    image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms) for x in ['query_drone_new', 'gallery_satellite', 'query_satellite', 'gallery_drone']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=False, num_workers=16) for x in ['query_drone_new', 'gallery_satellite', 'query_satellite', 'gallery_drone']}
use_gpu = torch.cuda.is_available()   # 'gallery_drone', 'gallery_street', 'query_satellite',, 'query_street'
...
def which_view(name):
    if 'satellite' in name:  # satellite
        return 1
    elif 'street' in name:
        return 2
    elif 'drone' in name:  # drone
        return 3
    else:
        print('unknown view')
    return -1
...
def extract_feature(model, dataloaders, view_index = 1):
    ...
    for data in dataloaders:
        ...
        count += n
        print(count)
        ff = torch.FloatTensor(n, 1024).zero_().cuda()   #512  1024
            ...
            for scale in ms:
                ...
                if opt.views == 2:
                    if view_index == 1:
                        ...
                        outputs, _, outputs_r, outputs_c, _, _ = model(input_img, None)
                        s = torch.cat([outputs_c, outputs_r], dim=1)
                    elif view_index == 3:  # 原来为2
                        # _, outputs = model(None, input_img)
                        _, outputs, _, _, outputs_r, outputs_c = model(None, input_img)
                        s = torch.cat([outputs_c, outputs_r], dim=1)
                elif opt.views == 3:
                    if view_index == 1:
...
# model.classifier_0.classifier = nn.Sequential()
model.classifier_1.classifier = nn.Sequential()
model.classifier_2.classifier = nn.Sequential()
model = model.eval()
if use_gpu:
    model = model.cuda()
...
gallery_label, gallery_path = get_id_g(gallery_path)
query_label, query_path = get_id_g(query_path)
...
if __name__ == "__main__":
    ...
    # scipy.io.savemat('pytorch_result.mat', result)  # pytorch_result

    # 多张
    result = {'gallery_f': gallery_feature.numpy(), 'gallery_label': gallery_label, 'gallery_path': gallery_path, 'mquery_f': query_feature.numpy(), 'mquery_label': query_label, 'query_path': query_path}
