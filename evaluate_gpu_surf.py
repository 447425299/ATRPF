...
def surf_features(img_paths):
    ...
    for img_path in img_paths:
        ...
        if des is not None:
            surf_features[img_path] = des
        else:
            surf_features[img_path] = None
    return surf_features

def surf_match(query_des, gallery_des):
...
gallery_surf_features = surf_features(gallery_paths)
...
if multi:
    for i in range(len(mquery_label)):
        ...
        if CMC_tmp[0] == 0:
            ...
            surf = cv2.xfeatures2d.SURF_create()
            kp_q, des_q = surf.detectAndCompute(query_img, None)
            if des_q is None:
                continue
            scores = []
            for img_path in gallery_paths:
                des_g = gallery_surf_features[img_path]
                score = surf_match(des_q, des_g)
