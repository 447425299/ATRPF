...
def get_model_list(dirname, key):
    if os.path.exists(dirname) is False:    # 如果path指向一个已存在的路径或已打开的文件描述符，返回True，对于失效的符号链接，返回False
...
def load_network(name, opt):
    # Load config
    dirname = os.path.join('./model', name)  # 目录名=  ./model拼接name
    last_model_name = os.path.basename(get_model_list(dirname, 'net'))  # 返回路径的基本名称
    epoch = last_model_name.split('_')[1]  # 返回net_119.pth中的119.pth
    epoch = epoch.split('.')[0]  # 得到119
    if not epoch == 'last':
       epoch = int(epoch)
    config_path = os.path.join(dirname, 'opts.yaml')
    with open(config_path, 'r') as stream:
        config = yaml.load(stream, Loader=yaml.FullLoader)  # 读取yaml文件
    ...
    opt.lr = config['lr']  # 学习率
    opt.nclasses = config['nclasses']  # 种类数
    ...
    if opt.PCB:
        model = PCB(opt.nclasses, True)

    if opt.views == 2:
        model = two_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share)   # 得到y1，y2
    elif opt.views == 3:
        model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share)   # 得到y1，y2，y3，y4
    ...
    network = model
    network.load_state_dict(torch.load(save_path))  # 加载之前保存的模型参数
