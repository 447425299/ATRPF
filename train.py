...
def train_model(model, model_test, criterion, optimizer, scheduler, num_epochs=25):
    ...
    warm_iteration = round(701 / opt.batchsize) * opt.warm_epoch  # first 5 epoch  dataset_sizes['satellite']
    ...
    if opt.instance:
        criterion_instance = losses.InstanceLoss(gamma=32)  # 64
    ...
    for epoch in range(num_epochs - start_epoch):
        ...
        for phase in ['train']:
            ...
            for data, data2, data3, data4 in zip(dataloaders['satellite'], dataloaders['street'], dataloaders['drone'],
                ...
                return_feature = opt.arcface or opt.cosface or opt.circle or opt.triplet or opt.contrast or opt.lifted or opt.sphere or opt.instance
                ...
                if opt.views == 2:
                    ...
                    if return_feature:
                        ...
                        loss = criterion(logits, labels) + 0 * criterion(logits2, labels2) + criterion(logits3, labels3)
                        ...
                        if opt.instance:
                            loss += criterion_instance(ff, labels) / now_batch_size + 0 * criterion_instance(ff2, labels2) / now_batch_size + criterion_instance(ff3, labels3) / now_batch_size
            ...
            epoch_loss = running_loss / 701  ## dataset_sizes['satellite']
            epoch_acc = running_corrects / 701  ## dataset_sizes['satellite']
            epoch_acc2 = running_corrects2 / 701  ## dataset_sizes['satellite']

            if opt.views == 2:
                print('{} Loss: {:.4f} Satellite_Acc: {:.4f}  Drone_Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc,
                                                                                         epoch_acc2))
            elif opt.views == 3:
                epoch_acc3 = running_corrects3 / 701 ## dataset_sizes['satellite']
